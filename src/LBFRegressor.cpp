...
void LBFRegressor::Train(const vector<Mat_<uchar> >& images,
    ...
    // data augmentation by multiple initialization 
    ...
    for(int i = 0;i < images.size();i++)
	{
        for(int j = 0;j < global_params.initial_num;j++)
		{
            ...
            // 2. Project current shape to bounding box of ground truth shapes  temp=[-1,-1]
			//use other ground truth shape to initial this sample,that compose a new train sample.
    ...
    // train random forest each keypoint has "max_numtrees" and each tree has 2^depth-1 feature node.
    ...
    for (int stage = 0; stage < global_params.max_numstage; stage++){
		...
		//obtain all train samples initial shape delta.
		...
		//only current_shapes are different,derive by other initial.
        RandomForest_[stage].Train(augmented_images,augmented_ground_truth_shapes, current_shapes,
			augmented_bounding_boxs, mean_shape_, shapes_residual_, stage);
        double t2 = (double)cvGetTickCount();
        cout << "the random forest of "<< stage<<" stage has been trained, cost "<< 
			(t2-t1)/((double)cvGetTickFrequency()*1000*1000) <<" s"<<endl<<endl;

        cout << "derive binary codes given learned random forest in stage"<< stage << endl;
        struct feature_node ** binfeatures ;
        binfeatures = DeriveBinaryFeat(RandomForest_[stage], augmented_images,
			current_shapes, augmented_bounding_boxs);
        double t3 = (double)cvGetTickCount();
        cout << "derive binary features of "<< stage<<" stage has been trained, cost "<< 
			(t3-t2)/((double)cvGetTickFrequency()*1000*1000) <<" s"<<endl<<endl;
        
        cout << "learn global linear regression given binary feature" << endl;
        GlobalRegression(binfeatures, shapes_residual_, current_shapes, augmented_bounding_boxs,
			mean_shape_, Models_[stage], num_feature, num_train_sample, stage);
        ...
        cout << "the linear model of "<< stage<<" stage has been trained, cost "<<
			(t4-t3)/((double)cvGetTickFrequency()*1000*1000) <<" s"<<endl<<endl;

        cout << "the "<<stage<<" has completed, cost "<<
			(t4-t0)/((double)cvGetTickFrequency()*1000*1000) <<" s"<<endl;
        cout << "Remaining time is about "<< 
			(t4-t0)/((double)cvGetTickFrequency()*1000*1000*(stage+1))*(global_params.max_numstage-stage-1)
    	<< "s"<<endl<<endl;
    }
}
